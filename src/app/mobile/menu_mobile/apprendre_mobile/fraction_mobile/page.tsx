  ...
  const operations: FractionOperation[] = [
    {
      ...
      description:
        "L'addition de fractions nécessite des dénominateurs identiques. Si les dénominateurs sont différents, il faut d'abord les rendre égaux.",
    ...
    },
    {
      ...
      description:
        "La soustraction de fractions suit la même règle que l'addition. On doit rendre les dénominateurs égaux si nécessaire.",
      formula: "A/B - C/B = (A - C)/B",
    ...
    },
    {
      ...
      example:
        "Si A/B = 3/4 et C/D = 1/2, on a : 3/4 ÷ 1/2 = 3/4 × 2/1 = 6/4 = 1 2/4 = 1 1/2",
    ...
    <div className="fixed inset-0 overflow-y-auto bg-gray-100 text-black">
      ...
      <main className="min-h-screen flex flex-col items-center p-4 pt-24 pb-28 relative">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold mb-4">Opérations sur les fractions</h1>
          <p className="text-lg">Sélectionne une opération pour apprendre à la réaliser :</p>
        </div>

        {/* Boutons des opérations (2 colonnes, compact) */}
        <div className="grid grid-cols-2 gap-3 mb-10 w-full max-w-md">
          {operations.map((operation, index) => (
            <button
              key={index}
              className="bg-blue-500 text-white py-2 px-4 rounded font-bold shadow-lg hover:bg-blue-700 transition-all duration-300"
