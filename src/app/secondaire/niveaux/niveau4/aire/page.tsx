  ...
  const generateQuestions = (total: number = totalQuestions) => {
    return Array.from({ length: total }, () => {
      ...
      if (shapeType === 0) {
        ...
        questionText = `Quelle est l'aire d'un rectangle de ${length} cm × ${width} cm ?`;
        ...
        // Losange : Aire = (diag1 × diag2) / 2
        const d1 = Math.floor(Math.random() * 10) + 1;
        const d2 = Math.floor(Math.random() * 10) + 1;
        questionText = `Quelle est l'aire d'un losange avec des diagonales de ${d1} cm et ${d2} cm ?`;
        correctAnswer = (d1 * d2) / 2;
      } else if (shapeType === 4) {
        // Parallélogramme : Aire = base × hauteur
        const base = Math.floor(Math.random() * 10) + 1;
        const height = Math.floor(Math.random() * 10) + 1;
        questionText = `Quelle est l'aire d'un parallélogramme de base ${base} cm et de hauteur ${height} cm ?`;
        ...
        const height = Math.floor(Math.random() * 10) + 1;
        questionText = `Quelle est l'aire d'un trapèze avec des bases de ${base1} cm et ${base2} cm, et une hauteur de ${height} cm ?`;
        correctAnswer = ((base1 + base2) * height) / 2;
      }
  
      return {
        questionText,
        correctAnswer: correctAnswer.toFixed(2), // 2 décimales pour plus de précision
      };
    });
  };
