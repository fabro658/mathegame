...
export default function Multiplication() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 text-black relative">
      ...
      <Link
        href="/menu/apprendre"
        className="absolute top-4 right-4 bg-orange-500 text-white py-2 px-4 rounded font-bold"
      >
        Apprendre
      </Link>
            ...
            {questions
              ...
              .map(([factor1, factor2], index) => (
                <div key={index} className="flex items-center gap-4">
                  <button className="bg-blue-500 text-white font-bold py-6 px-8 rounded w-full" disabled>
                    {factor1} × {factor2}
                  </button>
                  <input
                    type="number"
                    className="border border-gray-400 p-4 rounded w-full text-center text-black"
                    value={answers[currentPage * questionsPerPage + index] || ""}
                    onChange={(e) => handleChange(currentPage * questionsPerPage + index, e.target.value)}
                    style={{ appearance: 'none' }} // Enlève les petites flèches du champ input
                  />
                </div>
              ))}
          </div>
